...
var act = function() {
  ...
  function validateParams(params, cb) {
    ...
    // @TODO add some check to see it matches valid format
    ...
    // @INFO It seems that are supporting two different apis here for backwards compatablility
  ...
  function executeAppCall(params, cb) {
    // @INFO: 3 functions passing return values on to the next one as parameters,
    // when it gets to the end it calls the callback,
    // if at any time it fails it calls the callback
    ...
    //       if ('object' !== typeof res || res.status !== 200) {
