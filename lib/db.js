...
function getMongoConnectionUrl(cb) {
  if (process.env.FH_MONGODB_CONN_URL) {
    ...
    // Backwards compatible, hard coded localhost for local development
...
module.exports = function(cfg) {
  ...
  if (cfg && cfg.fhditch) {
    if (cfg.fhditch.host) {
      ditch_host = cfg.fhditch.host;
    }
    if (cfg.fhditch.port) {
      ditch_port = cfg.fhditch.port;
    }
  }
  return db;
};
...
var db = function(params, callback) {
  console.log('[fh-mbaas-api/lib/db.js:97] at start of db function');
  var CONST_CONN_STRING = "connectionString";
  if (!params.act) {
    throw new Error("'act' undefined in params. See documentation of $fh.db for proper usage");
  }
  // Type required for all but list operations, where we can list collections by omitting the type param
  if (!(params.type) && ['close', 'list', 'export', 'import', CONST_CONN_STRING].indexOf(params.act) === -1) {
    throw new Error("'type' undefined in params. See documentation of $fh.db for proper usage");
  }
  if (!appname) {
    throw new Error("Internal error - no appId defined");
  }
  ...
  console.log('[fh-mbaas-api/lib/db.js:114] Starting async waterfall');
  ...
  async.waterfall([
    ...
    console.log('Got fh-db');
    if (process.env.FH_MONGODB_CONN_URL) {
      console.log('[fh-mbaas-api/lib/db.js:139]  Getting fhdb.local_db');
      return fhdb.local_db(params, callback);
    } else {
      console.log('Not getting fhdb.local_db');
      //net_db should not be called by apps created using fh-webapp --
      if (process.env.FH_DB_PERAPP) {
        debug('attempt to use db per app with no data storage enabled');
        ...
        console.log('[fh-mbaas-api/lib/db.js:148] Using net_db');
        debug('using net_db (DITCH)');
